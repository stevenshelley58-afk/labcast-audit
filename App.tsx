...
export default function App() {
  ...
  const isComplete = audit.status === AuditStatus.COMPLETE && audit.report;

  const isError = audit.status === AuditStatus.ERROR;

  const isIdle = audit.status === AuditStatus.IDLE;

  // Use legacy audit data directly
  const displayReport = audit.report;
  const displayTraces = audit.traces;
  const displayMetadata = audit.metadata;
